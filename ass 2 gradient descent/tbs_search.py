...
Inputs:
- f: Function
- x/y/z: Inputs of function f 

Outputs: gradient vector
"""
def calculate_derivative(f, x, y, z):
    h = 1e-7
    grad1 = (f(x+h, y, z) - f(x, y, z))/h
    grad2 = (f(x, y+h, z) - f(x, y, z))/h
    grad3 = (f(x, y, z+h) - f(x, y, z))/h
    
    gradient_vector = np.array([grad1, grad2, grad3])
...
def gradsearch(f, X, tolerance):
    ...
    def fline(t):
        ...
        fline = f(Xn[0], Xn[1], Xn[2])
        return fline
    ##############################
    
    
    # Gradient descent variables
    f_values = np.array([f(X[0], X[1], X[2])])
    ...
    while True:
        ...
        # Calculate direction
        gradient_vector = calculate_derivative(f, X[0], X[1], X[2])
        ...
        f_values = np.append(f_values, f(X[0], X[1], X[2]))
